...
def get_args():
    ...
    # FedHUG (Uncertainty-Aware Adaptive FedHUG)
    parser.add_argument('--ua_uncertainty_beta', default=1.0, type=float,
                        help='weight for uncertainty-aware class distribution during synthesis')
    parser.add_argument('--ua_entropy_batches', default=2, type=int,
                        help='number of local train batches to estimate per-class entropy (uncertainty)')
    parser.add_argument('--ua_budget_alpha', default=1.0, type=float,
                        help='scaling factor for dynamic synthesis budget based on weight divergence')
    parser.add_argument('--ua_budget_min_scale', default=0.5, type=float,
                        help='min scale for dynamic synthesis budget')
    parser.add_argument('--ua_budget_max_scale', default=2.0, type=float,
                        help='max scale for dynamic synthesis budget')
    parser.add_argument('--ua_entropy_weight', default=0.3, type=float,
                        help='weight for local-model entropy maximization (confidence penalty) during synthesis')
    parser.add_argument('--ua_boundary_lambda', default=0.1, type=float,
                        help='weight for decision-boundary loss during synthesis')
