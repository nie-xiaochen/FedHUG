...
from gen_utils.generate_utils import DeepInversionHook, GenImageDataset, MultiTransform, reset_model, Generator
...
class ImageSynthesizer():
    ...
    def synthesize(self):
        ...
        if self.args.generator:
            ...
            inputs = torch.randn((targets.shape[0],self.args.channel, self.img_size, self.img_size), requires_grad=True, device="cuda")
        ...
        for it in range(self.iterations):
            ...
            loss_bn = sum([h.r_feature for h in hooks])  # bn loss

            # Loss_task(Global_Model): ensure teacher(global) predicts targets
            loss_task = F.cross_entropy(t_out, targets)

            # Loss_confidence(Local_Model): maximize entropy (discourage high-confidence predictions)
            self.student.eval()
            s_out = self.student(inputs_aug)
            p_local = F.softmax(s_out, dim=1)
            loss_confidence = -(p_local * torch.log(p_local + 1e-12)).sum(dim=1).mean()

            # Loss_boundary: push samples towards decision boundary (minimize top-1/top-2 margin)
            top2 = torch.topk(p_local, k=2, dim=1).values
            loss_boundary = (top2[:, 0] - top2[:, 1]).mean()

            # FedHUG objective:
            # Loss_total = Loss_task(Global_Model) - Loss_confidence(Local_Model) + λ * Loss_boundary (+ BN regularization)
            loss = self.bn * loss_bn + self.oh * loss_task - self.args.ua_entropy_weight * loss_confidence + self.args.ua_boundary_lambda * loss_boundary
            ...
            optimizer.step()
        try:
            print('loss_task:{}, loss_conf(entropy):{}, loss_boundary(margin):{}, loss_bn:{}'.format(loss_task.item(), loss_confidence.item(), loss_boundary.item(), loss_bn))
        except:
            best_inputs = inputs.data
            net.cuda()
